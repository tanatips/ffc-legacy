...
public class PersonunableNeedEdit extends VisitActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mPID = getIntent().getStringExtra(PersonunableNeed.PID);
        ...
        topic.setText(getString(R.string.pu_needcode));
    ...
    public void doSave() {
        ...
        for (String typecode : getDeleteList()) {
            Uri uri = PersonunableNeed.CONTENT_URI;
            ContentResolver cr = getContentResolver();
            int count = cr.delete(uri, "pid=? AND pcucodeperson=? AND needcode =?", new String[]{mPID, mPCUCODEPERSON, typecode});
        ...
        for (String tag : getEditList()) {
            ...
            if (f != null) {
                ...
                String code = et.getContentValues().getAsString(PersonunableNeed.NEEDCODE);
                if (isAddedCode(codeList, code)) {
                    finishable = false;
                    Toast.makeText(this, R.string.err_duplicate_need, Toast.LENGTH_SHORT).show();
                    break;
                } else
                    codeList.add(code);

                ContentValues cv = et.getContentValues();
                cv.put(PersonunableNeed.PID, mPID);
                cv.put(PersonunableNeed.PCUCODEPERSON, mPCUCODEPERSON);
                cv.put(PersonunableNeed.DATEUPDATE, DateTime.getCurrentDateTime());

                String action = f.action;
                if (action.equals(Action.INSERT)) {
                    et.commit(PersonunableNeed.CONTENT_URI);
                    f.action = Action.EDIT;
                    f.key = code;

                    System.out.println("I'M HERE");
                } else if (action.equals(Action.EDIT)) {
                    Uri updateUri = PersonunableNeed.CONTENT_URI;
                    et.commit(updateUri, "pid=? AND needcode=?", new String[]{mPID, f.key});
    ...
    public Loader<Cursor> onCreateLoader(int arg0, Bundle arg1) {
        ...
        setSupportProgressBarIndeterminateVisibility(true);
        CursorLoader cl = new CursorLoader(this, PersonunableNeed.CONTENT_URI,
                PROJECTION, "pid =? AND pcucodeperson=?", new String[]{mPID, mPCUCODEPERSON},
                PersonunableNeed.DEFAULT_SORTING);
        return cl;
    }
...
//		private String arg3;
        ...
        public View onCreateView(LayoutInflater inflater, ViewGroup container,
            ...
            View v = inflater.inflate(R.layout.pesonunable_need_fragment,
                    container, false);

            typecode = (SearchableButton) v.findViewById(R.id.type);
            typecode.setDialog(getFragmentManager(), NeedDialogList.class, "Problem?");
...
//			arg3 = args.getString("arg3");
        ...
        public boolean onSave(EditTransaction et) {
            // TODO Auto-generated method stub
            et.retrieveData(PersonunableNeed.NEEDCODE, typecode, false, null, null);

            ContentValues cv = et.getContentValues();
            cv.put(PersonunableNeed.NEEDCODE, typecode.getSelectId());
            return true;
        }
