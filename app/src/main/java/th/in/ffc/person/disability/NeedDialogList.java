 ...
 * @since 1.0
 */
public class NeedDialogList extends FFCSearchListDialog.BaseAdapter {


    private static final String[] PROJECTION = new String[]{PersonNeed._ID,
            PersonNeed._NAME,

    };

    private static final String[] FROM = new String[]{PersonNeed._NAME,
            PersonNeed._ID};
    ...
    public Loader<Cursor> onLoadCursor(String filter) {
        ...
        if (!TextUtils.isEmpty(filter)) {
            selection = "(" + PersonNeed._NAME + " LIKE '%" + filter + "%' or "
                    + PersonNeed._ID + " LIKE '" + filter + "%')";
        }

        CursorLoader cl = new CursorLoader(getActivity(), uri, PROJECTION,
                selection, null, PersonNeed._ID);
